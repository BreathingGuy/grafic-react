...
**Индекс дат** (`utils/dateIndex.js`) работает динамически:
- Генерация дат происходит **по запросу** для нужного года
- Нет жестких ограничений по диапазону годов
- Сгенерированные года кэшируются в памяти
- При инициализации предзагружается текущий год + 5 лет вперед

```javascript
// Автоматически генерирует год если его нет в кэше
const dates = getYearDates(2027);

// Управление кэшем
preloadYears(2025, 2030);  // Предзагрузка диапазона
clearDateCache();           // Очистка кэша
getCacheInfo();            // Информация о кэше
```

**Преимущества:**
- Можно создавать **любые годы** без изменения кода
- Экономия памяти — генерируются только используемые года
- Быстрый доступ за счет кэширования
...
- Кнопка "+ Новый год" в селекторе года
- Автоматически создает `max(availableYears) + 1`
- Генерирует пустые ячейки для всех сотрудников
- Включает **Q1 следующего года** для offset таблицы
- Пример: создание 2027 → ячейки для 2027-01-01...2027-12-31 + 2028-01-01...2028-03-31
